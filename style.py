...
def save_checkpoint(epoch, batch_num, model, optimizer, style_name):
    if not os.path.exists("checkpoints"):
        os.makedirs("checkpoints")
    checkpoint_path = f"checkpoints/{style_name}_dpws_checkpoint.pth"
    checkpoint = {
        'epoch': epoch,
        'batch_num': batch_num,
    ...
    print(f"\nCheckpoint saved at epoch {epoch+1}, batch {batch_num+1}")

def load_checkpoint(model, optimizer, style_name):
    checkpoint_path = f"checkpoints/{style_name}_dpws_checkpoint.pth"
    if os.path.exists(checkpoint_path):
        # 添加 map_location 参数，确保在 CPU 环境下也能加载
        ...
        print(f"\nCheckpoint loaded: resuming from epoch {epoch+1}, batch {batch_num+1}")
...
def train(args):
    # 正确的设备设置方式
    device = torch.device('cuda' if torch.cuda.is_available() and args.gpu != None else 'cpu')
    ...
    if (visualize):
        ...
        testImage_maine = testImage_maine.repeat(1, 1, 1, 1).requires_grad_(False).to(device)
        test_name = os.path.split(args.test_image)[-1].split('.')[0]

    image_transformer_dpws = ImageTransformNet_dpws().to(device)
    ...
    image_transformer.load_state_dict(torch.load(args.load_path, weights_only=False))
    ...
    start_epoch, start_batch = load_checkpoint(image_transformer_dpws, optimizer, style_name)
    if start_epoch is None:
        start_epoch = 0
        start_batch = -1

    for e in range(start_epoch, EPOCHS):
        batch_offset = start_batch + 1 if e == start_epoch else 0
        ...
        # 快速跳过batch
        if batch_offset > 0:
            print(f"Skipping {batch_offset} batches...")
        ...
        for batch_num, (x, label) in enumerate(train_loader_iter, batch_offset):
            ...
            aggregate_l1_loss += l1_loss.item()
            ...
            if ((batch_num + 1) % 5000 == 0):
                save_checkpoint(e, batch_num, image_transformer_dpws, optimizer, style_name)
                if visualize:
                    ...
                    utils.save_image(test_path, outputTestImage_maine[0].cpu())
...
def style_transfer(args):
    ...
    content = content.repeat(1, 1, 1, 1).requires_grad_(False).to(device)

    checkpoint_lw = torch.load(args.model_path, weights_only=False)

    style_model = ImageTransformNet_dpws().to(device)
    style_model.load_state_dict(checkpoint_lw)

    stylized = style_model(content).cpu()
    utils.save_image(args.output, stylized[0])
